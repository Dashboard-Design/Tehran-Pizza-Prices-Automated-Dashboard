...
def fetch_menu(retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"burgerland attempt {attempt + 1} failed: {e}")
...
def extract_italian_pizzas(data):
    ...
    target = next((cat for cat in categories if cat.get("title") == "سوپر پیتزا (30 سانتی متری)"), None)
    if not target:
        raise ValueError("Category 'سوپر پیتزا (30 سانتی متری)' not found")
    pizzas = []
...
def scrape() -> List[Dict]:
    ...
    data = fetch_menu()
    if not data:
        raise Exception("Failed to fetch burgerland menu after retries")

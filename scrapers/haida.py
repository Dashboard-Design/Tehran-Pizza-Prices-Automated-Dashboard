...
def fetch_menu(retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"khatoonpizaa attempt {attempt + 1} failed: {e}")
...
def extract_italian_pizzas(data):
    ...
    target = next((cat for cat in categories if cat.get("title") == "پیتزا 30 سانتی"), None)
    if not target:
        raise ValueError("Category 'پیتزا 30 سانتی' not found")
    pizzas = []
    for product in target.get("products", []):
        ...
        if variations:
            ...
            pizzas.append({"title": name, "price_toman": price})
    return pizzas
...
def scrape() -> List[Dict]:
    ...
    data = fetch_menu()
    if not data:
        raise Exception("Failed to fetch khatoonpizza menu after retries")

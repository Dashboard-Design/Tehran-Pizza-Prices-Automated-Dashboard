...
def fetch_menu(retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"haida attempt {attempt + 1} failed: {e}")
...
def extract_pizzas(data):
    ...
    target = next((cat for cat in categories if cat.get("title") == "پیتزا"), None)
    if not target:
...
def scrape() -> List[Dict]:
    ...
    data = fetch_menu()
    if not data:
        raise Exception("Failed to fetch haida menu after retries")
    # for testing uncomment below and add scrape() at the end
    # result = extract_pizzas(data)
    # for pizza in result:
    #     print(pizza["title"], pizza["price_toman"])
